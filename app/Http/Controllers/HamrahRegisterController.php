...
use App\Exports\HamrahExport;
use App\Models\Anjoman;
use App\Models\Fee;
use App\Models\Hamrah;
use App\Models\HamrahPay;
...
class HamrahRegisterController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('register-hamrah', compact(['anjomans']));
    }
    ...
    public function prepareData(Request $data)
    {
        ...
        $data->validate([
            ...
            'stdID' => 'required|digits_between:7,8',
    ...
    public function calculateBill(Request $request)
    {
        ...
        $hamrahanFee = 60000; // TODO: must be save in db
        $food = Fee::query()
        ...
        return view('hamrah-confirm', compact(['newPay', 'col', 'anjoman', 'food']));
    }

    public function payment($id)
    {
        $payment = HamrahPay::query()->findOrFail($id);
        $payment->update([
            'order_id' => $id + 100,
        ...
        if ($request->failed()) {
            ...
            Log::error('ERROR IN HAMRAH PAYMENT FUNCTION = ', $request);
        }
    }
    ...
    public function confirmPayment(CallbackRequest $request)
    {
        ...
        if ($payment->successful()) {
            ...
            $confirmPayment = HamrahPay::query()
            ...
            $this->createHamrah($confirmPayment);
    ...
    private function createHamrah(HamrahPay $payment)
    {
        Hamrah::query()->create([
    ...
    public function getReport($name)
    {
        if ($name == "YazdUniGetReport1400") { // TODO: This is a messy! Should be clean in next version
            return Excel::download(new HamrahExport, 'hamrahan.xlsx');
        }
        return redirect('/');
    }
